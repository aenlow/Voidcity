 ...
 */
export const GROWTH_SCALE = 0.175;
...
export class VoidEntity {
  constructor(opts = {}) {
    ...
    // Handicaps — 1 is "plays exactly like the player". Rivals get scaled down
    // on the easier difficulties instead of being made artificially stupid,
    // which keeps them readable: a slower rival still plays a sensible game.
    this.growthMult = opts.growthMult ?? 1;
  ...
  absorb(obj, massScale = 1) {
    ...
    this.mass += obj.mass * this.growthRate * this.growthMult * massScale * GROWTH_SCALE;
  ...
  update(dt, dir, bounds) {
    const speed = speedForRadius(this.radius, this.godzilla) * this.boost * this.speedMult;
